...
mis.q.DR<-function(data){
    ...
    evalq<-function(theta,data){
        as.numeric(exp(cbind(1,data$Z[,1])%*%theta[(nU+2):(nU+3)]))
    }
    ...
    gradv<-function(theta,data){
        ...
        out[nU+7,(nU+2):(nU+3)]<-colMeans(-q*(data$Y-h)*cbind(1,data$Z[,1])*(t<=T0))
...
mis.q<-function(data){
    ...
    gradv<-function(theta,data){
        ...
        #g4
        out[6,6]<-T0/T
        out[6,1:2]<-colMeans(-q*data$Y*cbind(1,data$Z[,1])*(t<=T0))
